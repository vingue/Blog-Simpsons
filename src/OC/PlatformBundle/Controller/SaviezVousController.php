...
use OC\PlatformBundle\Entity\Saviezvous;

class SaviezVousController extends Controller
{
    ...
    {
        return $this->render('OCPlatformBundle:Saviezvous:ajout.html.twig');
...
public function articleAction($id)
	{		
		$ent = $this->getDoctrine()->getRepository('OCPlatformBundle:Saviezvous')->find($id);
	...
	public function ajoutAction()
	{
		...
		return $this->redirect($this->generateUrl('saviez-vous_article', array('id'=>$ent->getId())));
	}
	...
	public function InitModifierAction($id)
	{
		...
		return $this->render('OCPlatformBundle:Saviezvous:modif.html.twig', array('title' => $ent->getTitle(),'contenu' => $ent->getContenu(), 'lien' => $url, 'id' => $ent->getId()));
		exit;
...
{
  $em = $this->getDoctrine()->getEntityManager();
  $acteur = $em->find('OCPlatformBundle:Saviezvous', $id);
        
  $em->remove($acteur);
  $em->flush();        

	return $this->redirect($this->generateUrl('saviez-vous_list'));
}
